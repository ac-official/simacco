...
if($_REQUEST['type'])  // receipts on job edit
...
$pdf->Cell(55, 8, "Pan Card No:  AABCU6074G", 0, 0, 'L', 1);
$pdf->Cell(140, 8, "Service Tax Reg.No: AABCU6074GSD001", 0, 0, 'R', 1);
    
$pdf->SetLineWidth(0.1);
$pdf->Line(11, 54, 204, 54);
$pdf->SetLineWidth(0.2);
$pdf->SetY(54);
$pdf->SetFont('Arial', '', 17);
$pdf->Cell(180, 8, "TAX INVOICE", 0, 0, 'C', 1);

$pdf->SetY(62);
$pdf->SetFont('Arial', '', 11);	
$pdf->Cell(90, 8, "Invoice No: ".$data['InvoiceID'], 0, 0, 'L', 1);
$pdf->Cell(100, 8, $data['DocumentID'] , 0, 0, 'R', 1);

$pdf->SetY(70);
$pdf->SetFont('Arial', '', 11);

$pdf->Cell(90, 8, "To : ".$data['FirstName'].",".$data['LastName'], 0, 0, 'L', 1);
$pdf->Cell(100, 8, "Date : ".$data['CashReceivedDate'] , 0, 0, 'R', 1);

$pdf->SetY(78);
$phoneData = $data['Mobile'];
if($data['Landline'])
    $phoneData .= " / ".$data['Landline'];
$pdf->Cell(45, 8, "Phone No :  ".$phoneData , 0, 0, 'L', 1);

$pdf->SetY(86);
$pdf->Cell(45, 8, "Email :  ".$data['Email'] , 0, 0, 'L', 1);
...
$pdf->SetWidths(array(145,40));
$pdf->SetAligns('R','R');
$pdf->Row(array("STATUTORY AMOUNT",$data['INVStatutory']));
$pdf->Row(array("SUB TOTAL",$data['INVSubTotal']));
$pdf->Row(array("SERVICE TAX@14%",$data['INVServiceTax']));
$pdf->Row(array("TOTAL",$data['INVTotalAmount']));
$pdf->SetAligns('L','L');

$pdf->SetY($pdf->GetY()+10);
$pdf->SetFont('Arial', '', 11);
$pdf->Cell(50, 6, "Amount in Words Rupees : ", 0, 0, 'L', 1);
$pdf->SetFont('Arial', 'B', 11);
$pdf->Cell(100, 6, $data['INVTotalWords'], 0, 0, 'L', 1);
...
$pdf->SetY($pdf->GetY()+10);
$pdf->SetFont('Arial', 'B', 11);
$pdf->Cell(185, 6, "For Muble Solutions PVT LTD", 0, 0, 'R', 1);

$pdf->SetY($pdf->GetY()+10);
$pdf->SetFont('Arial', '', 11);
$pdf->Cell(185, 6, "Authorised Signatory", 0, 0, 'R', 1);

$file_name = substr(mt_rand().mt_rand().mt_rand(),4,6)."_".date("H_i_s")."_TaxReceipt.pdf";
