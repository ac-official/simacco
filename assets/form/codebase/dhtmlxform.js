...
function dhtmlXForm(parentObj, data, skin) {
	...
	this._addItem = function(type, id, data, sId, lp, pos, insertAfter) {
		// id-index
                ...
                // Added for encrypting url from Connector & Server Filtering in Combo
                if (type == "combo") {   
                    if(data.connector && typeof(data.connector) != "undefined") {
                        data.connector = preTally.Initialize.encryptURL(data.connector);
                    }
                    if(data.serverFiltering && typeof(data.serverFiltering) != "undefined") {
                        data.serverFiltering = preTally.Initialize.encryptURL(data.serverFiltering);
                    }
		}
