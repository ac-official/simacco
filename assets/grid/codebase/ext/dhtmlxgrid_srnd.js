	...
	for (var i=0; i<this.rowsBuffer.length; i++)
		if (!this.rowsBuffer[i]){
			...
			if (this.callEvent("onDynXLS",[i,usedbuffer])){
				var self=this;
				this.load(this.xmlFileUrl+"&posStart="+i+"&count="+usedbuffer, function(){
				...
				if (res==-1){
					if (this.xmlFileUrl){
						...
						if (this.callEvent("onDynXLS",this._current_load)) {
							this.load(this.xmlFileUrl+"&posStart="+this._current_load[0]+"&count="+this._current_load[1], this._data_type);
                                                }
					}
					return;
				} else {
