...
class MasterReportClass {
    var $MasterReportArray;
    var $BranchStkRptArray;
    var $RptFilterItemArray;
    var $row_count;
    //----------------------------------------- Item Based Reports ----------------------------------------//
    ...
    /**
     * new Item based reports 
     * with verified and unverified data getting
     * BS.BS_Status 1 and 2
     * Created BY Bilin @ 9-12-2024
    */
    function reportItemDataBoth($filt,$stDate='',$enDate='') {      
        $dateFilt = '';
        ...
        $count=0;
        $this->MasterReportArray = array();
        $sql = "SELECT CAST( SUM( BS_Amount ) AS DECIMAL( 15, 2 ) ) AS BS_Amount ,IT.IT_Id, IT.IT_Name,IT.IT_Transfers,IT.IT_Business,MH.MH_Type,MH.MH_Name,SH.SH_Id,SH.SH_Name, LC.LC_Name,LC.LC_Id, BS.BS_Status 
            FROM  `balance_sheets` AS BS
            LEFT JOIN `items` IT ON BS.IT_Id = IT.IT_Id
            LEFT JOIN `sub_heads` AS SH ON IT.SH_Id = SH.SH_Id
            LEFT JOIN `main_heads` AS MH ON SH.MH_Id = MH.MH_Id
            ...
            WHERE  ".$filt."
            ".$dateFilt."
            GROUP BY IT.IT_Id, BS.BS_Status ORDER BY IT.IT_Name,BS.BS_Status ASC";
        $this->sql = $sql;
        $result = mysqli_query($GLOBALS['con'],$sql);
        if($result){
            $itisary = [];
            while($row = mysqli_fetch_object($result)) {
                ...
                if (!isset($itisary[$row->IT_Id])) {
                    $row->vBs_amount = ($row->BS_Status == 1) ? $row->BS_Amount: 0; // verified 
                    $row->uBs_amount = ($row->BS_Status == 2) ? $row->BS_Amount: 0; // unverified
                    $this->MasterReportArray[$count] = $row;
                    $itisary[$row->IT_Id] = $count;
                    $count++;
                } else {
                    $slno   = $itisary[$row->IT_Id];
                    if ($row->BS_Status == 1) {
                        $this->MasterReportArray[$slno]->vBs_amount = $row->BS_Amount;
                    } else {
                        $this->MasterReportArray[$slno]->uBs_amount = $row->BS_Amount;
                    }
                }
            }
        }
    ...
    /**
     * New function for verified and unverified balance_sheets amount 
     * based on the above function "reportItemBranchData"
     * 2 query execute and BS_Status parameters not send in the inputs
     * take bs_status 2 , 1 data change the group by section
     * re arrange the data return by the query based on result loop  
     * created By Bilin
    */
    function reportItmBrnchData($filt, $stDate='', $enDate='') {        
        $dateFilt = '';
        $dateFilt = '';
        ...
        $this->MasterReportArray    = array();
        $locary                     = [];
        $sql = "SELECT CAST( SUM( BS_Amount ) AS DECIMAL( 15, 2 ) ) AS BS_Amount ,IT.IT_Id, IT.IT_Name, SH.SH_Id, SH.SH_Name, LC.LC_Name, LC.LC_Id ,LC.LC_Status, BS.BS_Status  
            FROM `balance_sheets` AS BS LEFT JOIN `items` IT ON BS.IT_Id = IT.IT_Id 
            LEFT JOIN `sub_heads` AS SH ON IT.SH_Id = SH.SH_Id 
            LEFT JOIN `main_heads` AS MH ON SH.MH_Id = MH.MH_Id 
            LEFT JOIN `locations` AS LC ON BS.LC_Id = LC.LC_Id 
            WHERE  ".$filt." ".$dateFilt." GROUP BY LC.LC_Id, BS.BS_Status ORDER BY LC.LC_Name";
        $result = mysqli_query($GLOBALS['con'],$sql);        
        if($result){

            while($row = mysqli_fetch_array($result,MYSQLI_ASSOC)) {
                $ki     = $count;
                if (isset($locary[$row['LC_Id']])) {
                    $ki = $locary[$row['LC_Id']];
                } else {
                    $this->MasterReportArray[$ki]               = $row;
                    $this->MasterReportArray[$ki][0]            = [1=>0, 2=>0];
                }
                $this->MasterReportArray[$ki][0][$row['BS_Status']]  = $row['BS_Amount'];
                if ($ki == $count) {
                    $locary[$row['LC_Id']]    = $count; 
                    $count++;
                }
            }
           $monthSQL = "SELECT LC.LC_Id,BS.BS_Status,YEAR(BS.BS_Date) AS YEAR,MONTH(BS.BS_Date) AS MONTH, CAST( SUM(BS.BS_Amount) AS DECIMAL(15,2) ) AS BS_Amount 
            FROM  `balance_sheets` AS BS
            LEFT JOIN  `items` IT ON BS.IT_Id = IT.IT_Id
            LEFT JOIN  `sub_heads` AS SH ON IT.SH_Id = SH.SH_Id
            LEFT JOIN  `main_heads` AS MH ON SH.MH_Id = MH.MH_Id
            LEFT JOIN  `locations` AS LC ON BS.LC_Id = LC.LC_Id
            WHERE ".$filt." ".$dateFilt." 
            GROUP BY LC.LC_Id, MONTH( BS.BS_Date ),YEAR(BS.BS_Date), BS.BS_Status  
            ORDER BY LC.LC_Id, MONTH( BS.BS_Date ),YEAR(BS.BS_Date)  ";
            $this->sql =$monthSQL;
            $monthResult = mysqli_query($GLOBALS['con'],$monthSQL);
            if($monthResult){
                while($mRow = mysqli_fetch_object($monthResult)) {

                    if (isset($locary[$mRow->LC_Id])) {
                        $ki = $locary[$mRow->LC_Id];
                        $this->MasterReportArray[$ki][$mRow->BS_Status][$mRow->YEAR."-".$mRow->MONTH]= $mRow->BS_Amount;
                    }
                }
            }
        }
    ...
    function reportItemUserData($filt,$stDate='',$enDate='') {		
        ...
        $count=0;
        $this->MasterReportArray = array();
         $sql = "SELECT SUM( BS_Amount ) AS BS_Amount ,IT.IT_Id, IT.IT_Name,SH.SH_Id,SH.SH_Name,US.US_Id,CONCAT(`US_FName`,' ',`US_LName`) AS US_Name
                    FROM  `balance_sheets` AS BS
                        LEFT JOIN `items` IT ON BS.IT_Id = IT.IT_Id
                        LEFT JOIN `sub_heads` AS SH ON IT.SH_Id = SH.SH_Id
                        LEFT JOIN `main_heads` AS MH ON SH.MH_Id = MH.MH_Id
                        LEFT JOIN `users_auth` AS US ON BS.US_Id = US.US_Id
                            WHERE  ".$filt."
                                ".$dateFilt."
                                    GROUP BY BS.IT_Id,BS.US_Id ORDER BY US.US_FName";
        $result = mysqli_query($GLOBALS['con'],$sql);
        if($result){
            while($row = mysqli_fetch_object($result)) {
                $this->MasterReportArray[$count] = $row;
...
//        }
    ...
    function reportItemBasedData($stDate,$enDate,$filter_key,$sort,$pos,$cnt, $statuss = 1) {	
         $dateFilt = '';        
        if($stDate!='' && $enDate!=''){
            $stDate= date("Y-m-d", strtotime($stDate));
            $enDate= date("Y-m-d", strtotime($enDate));
      ...
      $sql_sum="SELECT SUM(BS.BS_Amount) AS SUM, BS.BS_Status FROM `balance_sheets` AS BS
                                LEFT JOIN `items` IT ON BS.IT_Id = IT.IT_Id
                                LEFT JOIN `sub_heads` AS SH ON IT.SH_Id = SH.SH_Id
                                LEFT JOIN `main_heads` AS MH ON SH.MH_Id = MH.MH_Id
                                LEFT JOIN `descriptions` AS DS ON BS.BS_Description = DS.DS_Id
        ...
        if ($statuss == 1 || $statuss == 2) { 
            $sql_sum .= "AND BS.BS_Status = ".$statuss;
        } else { // status based checking added verified to verified and unverified setup 16-12-2025
             $sql_sum .= " AND (BS.BS_Status = 1 OR BS.BS_Status = 2) GROUP BY BS.BS_Status ORDER BY BS.BS_Status";
        }
        $unv_sum        = 0;  
        $this->row_sum  = 0;                             
       $result_sum=mysqli_query($GLOBALS['con'],$sql_sum);
        if ($statuss == 1 || $statuss == 2) {
            $row_sum        = mysqli_fetch_object($result_sum);        
            $this->row_sum  = round($row_sum->SUM,2);
        } else { // 16-12-2025
            while ($row_sum = mysqli_fetch_object($result_sum)) {
                $this->row_sum  = ($row_sum->BS_Status == 1) ? round($row_sum->SUM,2): $this->row_sum;
                $unv_sum        = ($row_sum->BS_Status != 1) ? round($row_sum->SUM,2): $unv_sum;
            }
        }
        $sql    = "SELECT SQL_CALC_FOUND_ROWS BS.US_Id, BS.BS_Id,BS.BS_MinAmount,BS.BS_MaxAmount, BS.BS_Date  , 
                    BS.BS_Amount, DS.DS_Description,BS.CHQ_Number, TR.TR_Id ,TR.TR_Track, 
                    IT.IT_Id, IT.IT_Name,IT.IT_Business, IT.SH_Id, 
                    SH.SH_Name,SH.SH_Track, MH.MH_Type, LC.LC_Name,US.US_FName,US.US_LName, BS.BS_Status
                                FROM `balance_sheets` AS BS
                                LEFT JOIN `items` IT ON BS.IT_Id = IT.IT_Id
                                LEFT JOIN `sub_heads` AS SH ON IT.SH_Id = SH.SH_Id
                                LEFT JOIN `main_heads` AS MH ON SH.MH_Id = MH.MH_Id
                                ...
                                ".$dateFilt."    
                                AND MH.MH_Type IN ('1','2') ";
        if ($statuss == 1 || $statuss == 2) { // 16-12-2025
            $sql    .= " AND BS.BS_Status =".$statuss;
        } else {
            $sql    .= " AND (BS.BS_Status = 1 OR BS.BS_Status = 2) ";
        }
        $sql        .= " ".$sort." LIMIT ".$pos.",".$cnt ;
        $result = mysqli_query($GLOBALS['con'],$sql);
        $result_count=mysqli_query($GLOBALS['con'],"SELECT FOUND_ROWS() AS COUNT");
        $result_row=mysqli_fetch_object($result_count);        
        $this->row_count=$result_row->COUNT;
        if($result){
            while($row = mysqli_fetch_object($result)) {
                $this->MasterReportArray[$count] = $row;
                $count++;
...
//        $count=0;
//        $this->MasterReportArray = array();
//        $sql = "SELECT SUM( BS.BS_Amount ) AS BS_Amount ,BS.LC_Id,IT.IT_Id, IT.IT_Name,IT.IT_Transfers,IT.IT_Business,
//                    MH.MH_Type,MH.MH_Name,SH.SH_Id,SH.SH_Name
//                    FROM `balance_sheets` AS BS
//                        LEFT JOIN `items` IT ON BS.IT_Id = IT.IT_Id
//                        LEFT JOIN `sub_heads` AS SH ON IT.SH_Id = SH.SH_Id
//                        LEFT JOIN `main_heads` AS MH ON SH.MH_Id = MH.MH_Id
//                            WHERE  ((BS.LC_Id = '".$LCId."' AND BS.PM_Id =1) OR (BS.BA_Id IN(11,12,15,23,27,28,59) AND (BS.PM_Id =2)))  
//                                    AND BS.BS_Status = 1 
//                                    AND ".$filt."
//                                    ".$dateFilt."
//                                        GROUP BY IT.IT_Id ORDER BY IT.IT_Name";
...
//    }
    ...
    function entryTypeSumItemRpt($filt,$stDate='',$enDate='',$statswise=0) { // get total amount for entry based types in Item Based Reports(grid footer data)
        $dateFilt = '';
        ...
        if ($statswise == 1) { //09-12-2024
            $sql .= ", BS.BS_Status ";
        }
        $sql .= " FROM  `balance_sheets` AS BS   
                LEFT JOIN `items` IT ON BS.IT_Id = IT.IT_Id  
                LEFT JOIN `sub_heads` AS SH ON IT.SH_Id = SH.SH_Id   
                LEFT JOIN `main_heads` AS MH ON SH.MH_Id = MH.MH_Id   
                LEFT JOIN `locations` AS LC ON BS.LC_Id = LC.LC_Id   
                WHERE  ".$filt." ".$dateFilt;
        if ($statswise == 1) { //09-12-2024
            $sql .= " GROUP BY BS.BS_Status ";
        }
        $result = mysqli_query($GLOBALS['con'],$sql);
        if ($statswise == 1) { //09-12-2024
            $retdata = ['InternalTransferReceived'=>0, 'InternalTransferPaid'=>0, 'BusinessReceived'=>0, 'BusinessReturned'=>0, 'Income'=>0, 'Expense'=>0, 'uv_itrans_rcvd'=>0, 'uv_itrans_paid'=>0, 'uv_busi_rcvd'=>0, 'uv_busi_retd'=>0, 'uv_income'=>0, 'uv_expense'=>0];
            while($row = mysqli_fetch_object($result)) {
                if ($row->BS_Status == 1) {
                    $retdata['InternalTransferReceived']= $row->InternalTransferReceived;
                    $retdata['InternalTransferPaid']    = $row->InternalTransferPaid;
                    $retdata['BusinessReceived']        = $row->BusinessReceived;
                    $retdata['BusinessReturned']        = $row->BusinessReturned;
                    $retdata['Income']                  = $row->Income;
                    $retdata['Expense']                 = $row->Expense;
                } else {
                    $retdata['uv_itrans_rcvd']  = $row->InternalTransferReceived;
                    $retdata['uv_itrans_paid']  = $row->InternalTransferPaid;
                    $retdata['uv_busi_rcvd']    = $row->BusinessReceived;
                    $retdata['uv_busi_retd']    = $row->BusinessReturned;
                    $retdata['uv_income']       = $row->Income;
                    $retdata['uv_expense']      = $row->Expense;
                }
            }
            return $retdata;
        }
    ...
    function reportBMRItemBasedData($stDate,$enDate,$filter_key) {	
         $dateFilt = '';        
        if($stDate!='' && $enDate!=''){
            $stDate= date("Y-m-d", strtotime($stDate));
            $enDate= date("Y-m-d", strtotime($enDate));
     ...
     $sql="SELECT  BS.US_Id, BS.BS_Id,BS.BS_MinAmount,BS.BS_MaxAmount, BS.BS_Date  , 
                    ...
                    SH.SH_Name,SH.SH_Track, MH.MH_Type, LC.LC_Name,US.US_FName,US.US_LName
                                ...
                                ".$dateFilt."    
                                AND MH.MH_Type IN ('1','2') 
                                AND BS.BS_Status = 1 ORDER BY IT.IT_Name";
        $result = mysqli_query($GLOBALS['con'],$sql);        
        if($result){
            while($row = mysqli_fetch_object($result)) {
                $this->MasterReportArray[$count] = $row;
