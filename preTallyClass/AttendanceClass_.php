...
//        echo $sql;
    ...
    //-----------------------------------------Sign out----------------------------------------//
    function MarkExit($id, $date, $time) {
        $sql = "UPDATE attendance SET AT_SignOut='" . $time . "',AT_Status =1 , AT_Hours = ROUND(time_to_sec(TIMEDIFF('" . $time . "',AT_SignIn))/60,2) WHERE US_Id=" . $id . " AND AT_Date='" . $date . "'";
        mysqli_query($GLOBALS['con'], $sql);
    ...
    function alterAttendance($usid, $date, $session) {
        ...
        if ($result['US_LoginTime'] != "" && $result['US_LogoutTime'] != "" && $result['US_WrkHours'] != "") {
            ...
            $sqlupdate = 'UPDATE attendance SET AT_SignIn="' . $loginTime . '",AT_SignOut="' . $logoutTime . '",AT_Hours=' . $wrkHrs . ' WHERE US_Id="' . $usid . '" AND AT_Date="' . $date . '"';
    ...
    function viewHolidays($filt = "") {
        ...
        $result = mysqli_query($GLOBALS['con'], "SELECT * FROM holidays " . $filt . " ORDER BY HD_Date ASC");
        while ($row = mysqli_fetch_object($result)) {
    ...
    function checkHoliday($date) {
        $result = mysqli_query($GLOBALS['con'], "SELECT COUNT(*) FROM holidays WHERE HD_Date=" . $date);
    ...
    //----------------------------------------- Fetch all Holidays ----------------------------------------//
    function getHolidays($date, $ofid) {
        ...
        $count = 0;
        $this->Holidays = array();
        $holsql = "SELECT ST_Id,HD_Date,HD_Comments FROM holidays WHERE HD_Date BETWEEN '" . $frmdate . "' AND '" . $lstdate . "' AND OF_Id=" . $ofid . " AND HD_Type!=3 AND HD_Status=1 ORDER BY ST_Id";
    ...
    function countReptAttendance($month, $year, $ofid, $halfday, $filter) {
        ...
        $sql_user = "SELECT COUNT(US.US_Id) AS CNT FROM `users_auth` AS US LEFT JOIN locations AS LC ON LC.LC_Id = US.LC_Id WHERE (LAST_DAY(US.US_ResignDate)>='" . $lastday . "' OR US.US_ResignFlag = 0) AND US.US_DOJ<='" . $lastday . "' AND US.OF_Id=" . $ofid . " " . $filter;
    ...
    function listReptAttendance($month, $year, $ofid, $halfday, $filter, $pos, $cnt, $usid, $wrkgraceTime, $orderby) {
        ...
        $sql_user = "SELECT US.US_Id, US.OF_Id,US.DP_Id, LC.LC_Name,LC.ST_Id, US.US_FName, US.US_LName,US.US_LoginTime,US.US_LogoutTime,US.US_WrkHours 
                FROM `users_auth` AS US LEFT JOIN locations AS LC ON LC.LC_Id = US.LC_Id WHERE (LAST_DAY(US.US_ResignDate)>='" . $lastday . "' OR US.US_ResignFlag= 0) AND US.US_DOJ<='" . $lastday . "' AND US.OF_Id=" . $ofid . " " . $filter . " " . $userFilter . " ORDER BY " . $orderby . " LIMIT " . $pos . "," . $cnt;
        ...
        $sql = "SELECT US.US_Id,AT.AT_Date,AT.AT_SignIn,IF(US_WrkHrFlag=1 AND AT.AT_Date>US.US_WrkHrDate,US.US_LogOutTime,AT.AT_SignOut) AS AT_SignOut,IF(US_WrkHrFlag=1 AND AT.AT_Date>US.US_WrkHrDate,US.US_WrkHours,AT.AT_Hours) AS AT_Hours,IF(US_WrkHrFlag=1 AND AT.AT_Date>US.US_WrkHrDate,1,AT.AT_Status) AS AT_Status "
                ...
                . " AND IF(US_WrkHrFlag=1 AND AT.AT_Date>US.US_WrkHrDate,AT.AT_Hours>=0,AT.AT_Hours>=((US.US_WrkHours/2)-$wrkgraceTime)) ";
        $result = mysqli_query($GLOBALS['con'], $sql);
        while ($row = mysqli_fetch_assoc($result)) {
        ...
        $sql_user = "SELECT US.US_Id, US.US_EMPID,US.OF_Id,US.DP_Id, LC.LC_Name,LC.ST_Id, US.US_FName, US.US_LName,US.US_AttndFlag,US_LoginTime,US.US_LogoutTime,US.US_WrkHours,
                ...
                SAL.SS_Id,ST.SS_Status 
                    ...
                    WHERE (LAST_DAY(US.US_ResignDate)>='" . $lastday . "' OR US.US_ResignFlag = 0) 
                     AND US.US_DOJ<='" . $lastday . "' AND US.OF_Id=" . $ofid . " ORDER BY US.US_Id";
    ...
    function empSalPayroll($month, $year, $ofid, $halfday) {
        ...
        $sql_user = "SELECT US.US_Id, US.US_EMPID,US.US_FName, US.US_LName,SAL.US_GrossSal,ST.SS_Status
                    ...
                    WHERE ((MONTH(US.US_ResignDate)>=MONTH('" . $lastday . "') AND YEAR(US.US_ResignDate)>=YEAR('" . $lastday . "')) OR US.US_ResignFlag= 0) AND                     
                     US.US_DOJ<='" . $lastday . "' AND US.OF_Id=" . $ofid . " ORDER BY US.US_Id";
        $result_user = mysqli_query($GLOBALS['con'], $sql_user);
        while ($row_user = mysqli_fetch_assoc($result_user)) {
...
//---------------------Select Dates of approved leave of selected month----------------------------
    ...
    function getMailId($filter) {
        ...
        $query = "SELECT UA.US_Email,ESR.ESR_Id,ESR.US_FName,ESR.US_LName
                    ...
                    ON ESR.US_Id= UA.US_Id WHERE ESR.ESR_Id IN(" . $filter . ")";
        $result = mysqli_query($GLOBALS['con'], $query);
        while ($row = mysqli_fetch_object($result)) {
    ...
    function salarySlipPdfData($password) {
       ...
       $query = "SELECT UA.US_EMPID,ESR.US_FName, ESR.US_LName, DS.DG_Name, DM.DP_Name, ESR.ESR_Month, ESR.ESR_Year, ESR.US_BasicSal, ESR.US_HRASal, ESR.US_ConveySal, ESR.US_MedSal, ESR.US_GrossSal, ESR.ESR_Lop, ESR.ESR_Id, ESR.US_DedEPF, ESR.US_DedESI, ESR.US_DedLWF, AC.US_AccNo, AC.US_Bankname, AC.US_BankBranch,ESR.ESR_Proftax,
                            ...
                            ESR.ESR_TotWrkDays,ESR.ESR_PrsntDays,ESR.ESR_HlfDays,ESR.ESR_LOPDays,OF.OF_Logo,OF.OF_Name   
...
//                ESR_Month=".$month." AND OF_Id=".$preTally_user_ofid." AND ESR_Year=".$year." ".$filter." ". $order ." LIMIT ".$posStart.",".$end;
    ...
    function getBSSalReport($month, $preTally_user_ofid, $year, $salItem, $esrids) {
        ...
        if (is_numeric($month)) {
            $query = "SELECT ESR.ESR_Id,ESR.US_Id,ESR.US_FName,ESR.US_LName,
                    ...
                    AND ESR_Month=" . $month . " AND ESR.OF_Id=" . $preTally_user_ofid . " AND ESR.ESR_Year=" . $year . " AND ESR.ESR_Id IN (" . $esrids . ") GROUP BY ESR.US_Id";
            $result = mysqli_query($GLOBALS['con'], $query);
            while ($row = mysqli_fetch_assoc($result)) {
    ...
    function countPaymodeWiseEmpSalRpt($month, $year, $sel_modeOfPay, $ofid, $BA_Id, $filters) {
        ...
        if (is_numeric($month) && is_numeric($sel_modeOfPay)) {
            ...
            WHERE ESR.OF_Id=" . $ofid . " AND ESR.ESR_Year=" . $year . " AND ESR_Month=" . $month . " AND " . $sel_modeOfPayment . $filter;
            $result = mysqli_query($GLOBALS['con'], $query);
            while ($row = mysqli_fetch_object($result)) {
    ...
    function getWeekendOffs($month, $year, $ofid) {
        $this->WeekOffs = array();
        $date = $year . "-" . $month . "-01";
        $last = date('Y-m-t', strtotime($date));
        $lstdate = date('Y-m-d', strtotime('+5 days', strtotime($last)));
        ...
        $count = 0;
        $result = mysqli_query($GLOBALS['con'], "SELECT DH.DP_Id,DH.DH_Weekends FROM department_holidays as DH,departments AS DP WHERE DP.DP_Id=DH.DP_Id AND DP.OF_Id=" . $ofid);
        while ($row = mysqli_fetch_assoc($result)) {
            $dept_id = $row["DP_Id"];
    ...
    function createRHBatch($ofid) {
        $sql = "SELECT HD_Batch FROM holidays WHERE OF_Id=" . $ofid . " AND HD_Type=3 ORDER BY HD_Id DESC LIMIT 1";
        $result = mysqli_query($GLOBALS['con'], $sql);
        $row = mysqli_fetch_array($result, MYSQLI_NUM);
        if ($row[0] != "") {
    ...
    function getRHDays($batchId, $date) {
        $count = 0;
        $this->RH_Days = array();
        $sql = "SELECT HD_Id,HD_Date  FROM holidays WHERE HD_Batch ='" . $batchId . "' AND HD_Date !='" . $date . "'";
        $result = mysqli_query($GLOBALS['con'], $sql);
        while ($row = mysqli_fetch_assoc($result)) {
    ...
    function getOfficeRH($ofid) {
        $count = 0;
        $this->RH_Holidays = array();
        $result = mysqli_query($GLOBALS['con'], "SELECT ST_Id,HD_Date,HD_Batch FROM holidays WHERE OF_Id=" . $ofid . " AND HD_Type=3 AND HD_Status = 1  ORDER BY ST_Id,HD_Date");
        while ($row = mysqli_fetch_assoc($result)) {
            $arr_states = array();
            $arr_states = explode(',', $row['ST_Id']);
            foreach ($arr_states as $state_id) {
    ...
    function getBatchByDate($date, $ofid) {
        $sqlsel = "SELECT HD_Batch FROM holidays WHERE OF_Id=" . $ofid . " AND HD_Type=3  AND HD_Date='" . $date . "'";
    ...
    function listUser($ofId, $mask) {
        $sql = "SELECT CONCAT(US.US_FName,' ', US.US_LName) AS Name,US.US_Id FROM users_auth AS US WHERE US.OF_Id=" . $ofId . " AND US.US_Status!=0" . $mask . "LIMIT 0,10";
        $result = mysqli_query($GLOBALS['con'], $sql);
        while ($row = mysqli_fetch_object($result)) {
    ...
    function salAdvnceDetails($ofId, $pos, $cnt, $filter) {
        ...
        $sql = "SELECT SA.*,CONCAT(US.US_FName,' ', US.US_LName) AS Name,CONCAT(UA.US_FName,' ', UA.US_LName) AS CName FROM salary_advance_payment AS SA 
                ...
                WHERE US.US_Status!=0 AND US.OF_Id=" . $ofId . $filter .
    ...
    function repaymentDetails($ofId, $filt, $pos, $cnt) {
        ...
        $sql = "SELECT RS.* ,CONCAT(US.US_FName,' ', US.US_LName) AS Name,LC.LC_Name FROM repayment_schedule AS RS
                ...
                LEFT JOIN users_auth AS US ON US.US_Id=SA.US_Id  
                LEFT JOIN locations AS LC ON LC.LC_Id=US.LC_Id  WHERE US.US_Status!=0 AND US.OF_Id=" . $ofId . $filt;
        $result = mysqli_query($GLOBALS['con'], $sql);
        while ($row = mysqli_fetch_object($result)) {
            $this->salAdvnceDetailsArray[$count] = $row;
            $count++;
    ...
    function getMonthwiseSalRpt($filt, $stDate = '', $enDate = '', $ESR_Month_From, $ESR_Year_From, $ESR_Month_To, $ESR_Year_To, $ofId) {
        ...
        $query = "SELECT US.US_Id,CONCAT(US.US_FName,' ',US.US_LName) AS Name,LC.LC_Name  FROM users_auth AS US
               LEFT JOIN locations AS LC ON LC.LC_Id = US.LC_Id  WHERE US.OF_Id=" . $ofId . " ORDER BY  LC.LC_Name ASC";
    ...
    function listSalaryHistory($ofid, $filter, $orderBy, $pos, $cnt) {
        ...
        $query = "SELECT UA.US_Id,CONCAT(UA.US_FName,' ',UA.US_LName) AS Name,UA.US_DOJ,LC.LC_Name,US.US_GrossSal,UA.US_EMPID,DG.DG_Name,AL.ALC_Name,DP.DP_Name FROM users_auth AS UA                 
                ...
                WHERE UA.OF_Id=" . $ofid . " " . $filter . $orderBy . " LIMIT " . $pos . "," . $cnt;
        $result = mysqli_query($GLOBALS['con'], $query);
        while ($row = mysqli_fetch_object($result)) {
...
WHERE
  US.US_Id ='" . $id . "'";
    ...
    function listHolidays($ofid, $fromDate, $toDate, $stateId) {
        $indx = 0;
        $this->HolidayArray = array();
...
ST_Id)) AND HD_Date BETWEEN '".$fromDate."' AND '".$toDate."'";
//        echo $sql;
        $result = mysqli_query($GLOBALS['con'], $sql);
        while ($row = mysqli_fetch_object($result)) {
    ...
    function listRHTaken($usid, $fromDate) {
        $indx = 0;
        $this->RH_TaknDays = array();
        $sql = "SELECT
  RHL.RHL_Id AS ID,
...
//        echo $sql;
    ...
    function listAllRH($ofid, $fromDate, $toDate, $btch) {
        $indx = 0;
        $this->All_RH = array();
        $sql = "SELECT HD_Id,HD_Date,HD_Comments FROM holidays WHERE OF_Id='" . $ofid . "' AND HD_Type=3 AND HD_Status = 1 AND " . $btch . " AND HD_Date BETWEEN '" . $fromDate . "' AND '" . $toDate . "'   ORDER BY ST_Id,HD_Date;";
//        echo $sql;
        $result = mysqli_query($GLOBALS['con'], $sql);
        while ($row = mysqli_fetch_object($result)) {
