$(document).ready(function(e) {
	...
	function track(){
		if(window.localStorage){var t=signInForm.getItemValue("username"),e=u(),a={},r=[];if(null==(_oldPersist=localStorage.getItem("track")))r.push(btoa(t)),a[e]=r,localStorage.setItem("track",JSON.stringify(a));else{if(void 0!==(_oldPersist=JSON.parse(localStorage.getItem("track")))[e]){var n=t,s=_oldPersist[e];s.includes(btoa(n))||(s.push(btoa(n)),_oldPersist[e]=s,localStorage.setItem("track",JSON.stringify(_oldPersist)))}else r.push(btoa("arun1")),_oldPersist[e]=r,localStorage.setItem("track",JSON.stringify(_oldPersist));i()}function u(){var t,e=new Date,a=String(e.getDate()).padStart(2,"0");return a+"-"+String(e.getMonth()+1).padStart(2,"0")+"-"+e.getFullYear()}function i(){var t=Math.floor(Math.random()*Math.floor(Math.random()*Date.now())),a=localStorage.getItem("today");null==a?(localStorage.setItem("today",e),localStorage.setItem("uuid",t)):a!=e&&localStorage.setItem("uuid",t)}}
	}
