...
$order_by="IT.IT_Name,LC.LC_Name ASC";
if($REQUEST['amtSort']=='ASC' || $REQUEST['amtSort']=='DESC'){
$order_by="BS_Amount ".$REQUEST['amtSort'];    
}
...
if ($REQUEST['USId']!="" && $REQUEST['USId']!="null")
    $filter .= ' AND BS.BS_IEByUS = ' . $REQUEST['USId'] . ' ';
if ($REQUEST['LCId']!="" && $REQUEST['LCId']!="null" && $REQUEST['LCId']!="0")
...
$MstrRptObj->reportUserBasedDataCount($REQUEST['f'], $REQUEST['t'], $filter);
$MstrRptObj->reportUserBasedData($REQUEST['f'], $REQUEST['t'], $filter,$order_by,$_GET["posStart"],$_GET["count"]);
$MR_Obj = $MstrRptObj->MasterReportArray;
$TotCount = $MstrRptObj->MetaData['Count'];

echo("<?xml version='1.0' encoding='iso-8859-1'?>\n");
echo '<rows total_count="' . $TotCount . '" pos="' . $_GET["posStart"] . '">';
...
echo    '<userdata name="FromDate">' . date('d.m.Y', strtotime($REQUEST['f'])) . '</userdata>
    ...
    <userdata name="UBDMRSum">' .  $MstrRptObj->MetaData['Sum'] . '</userdata>    
    <userdata name="TL_Count">' . $TotCount . '</userdata>';
...
echo '<head>
            <afterInit>
                    <call command="setNumberFormat"><param>0,000.00,4</param></call>
                    <call command="setColumnMinWidth"><param>,' . $cellFormat . ',,</param></call>
                    <call command="attachFooter"><param><![CDATA[Total,#cspan,<div class="RMR_UBD">'.$MstrRptObj->MetaData['Sum'].'</div>,,,,]]></param>
...
if ($MR_Obj) {
    ...
    foreach ($MR_Obj as $rw) {
        ...
        echo '
                <row id="' . $rw->BS_Id . '">
                    ...
                    <userdata name="BS_IEByLC">' . $LCId . '</userdata>    
                    <userdata name="BS_IEByUS">' . $rw->BS_IEByUS . '</userdata>     
    ...
    echo '<row id="0"><cell colspan="5"><![CDATA[<div style="font-size:16px;font-variant:small-caps;color:#0979B1;text-align:center !important;font-family: serif;padding-top: 10px;" >No records found.</div>]]></cell> <cell></cell><cell></cell></row>';
}
echo '</rows>';
?>