...
$BSRptObj->reportNetProfitData($filter,$REQUEST['f'],$REQUEST['t'],$preTally_user_ofid);
...
foreach($MRP_Obj as $rw) {
    for($y = $toYear; $y >= $fromYear; $y--) {
        ...
        for($i = $monthLimit; $i >= $month; $i--) {
            ...
            $NetProfitArray[$rw->LC_Id][$y][$i] = $GrossArray[$rw->LC_Id][$y][$i] - (round($BSRptObj->FixedMonthReportArray[$rw->LC_Id][$y][$i]) + round($BSRptObj->VarMonthReportArray[$rw->LC_Id][$y][$i]) + round($BSRptObj->GenMonthReportArray[$rw->LC_Id][$y][$i]));
            $monthLimit--;
        }
    }
}
...
echo("<?xml version='1.0' encoding='iso-8859-1'?>\n");
    echo '<rows>';
    echo '<head>
        <column width="50"  type="ro" align="center" ><![CDATA[<div style="text-align:left;"> SlNo <img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="0" class="netInc_Sort_BIR"/></div>]]></column>
        <column width="120"  type="ro" align="left" ><![CDATA[<div><div id="branchIncNet" style="width: 70%;float:left;" placeholder="Branch"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="1" class="netInc_Sort_BIR"/></div></div>]]></column>';
    if($MRP_Obj) {
        ...
        for($y = $toYear; $y >= $fromYear; $y--) {
            ...
            for($i = $monthLimit; $i >= $month; $i--) {
                ...
                echo '<column width="*" format = "0,000" type="ron" align="right" ><![CDATA[<div style="float:left;">'.$monthArray[$monthLimit] .'<br>'.$y .'<img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="'.$colNum.'" class="netInc_Sort_BIR"/> </div>]]></column>';
                $colNum++;
        ...
        echo '<column width="140" format = "0,000" type="ron" align="right" ><![CDATA[<div style="float:left;"> Total <img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="'.$colNum.'" class="netInc_Sort_BIR" /> </div>]]></column>
            <afterInit>
        ...
        foreach($MRP_Obj as $rw) {
            ...
            echo '<row id = "'.$rw->LC_Id.'">
                    ...
                    for($y = $toYear; $y >= $fromYear; $y--) {
                        ...
                        $arr_amts   =  arrayFlatten($NetProfitArray[$rw->LC_Id]);                        
                        $maxamt     =  0;
                        $maxamt     =  max($arr_amts);
                        for($i = $monthLimit; $i >= $month; $i--) {
                            if($monthLimit == 0) 
                            ...
                            if($maxamt == $NetProfitArray[$rw->LC_Id][$y][$i] && $maxamt > 0)
                                $bgcolor = 'style="background-color:#33cc33;"';
                            else
                                $bgcolor = ''; 
                            echo '<cell name="month'.$y.$i.'" title= " " '.$bgcolor.'>';
                            echo $business = ($NetProfitArray[$rw->LC_Id][$y][$monthLimit]) ? $NetProfitArray[$rw->LC_Id][$y][$monthLimit] : 0;
