...
$filter         = ' PL.PL_Status = 1 AND AJ.AJ_Status != 0 AND AJ.AJ_Status != 2 AND PL.PL_Id != "NULL" AND AJ.OF_Id = '.$preTally_user_ofid;
$fields         = " CT.CT_Id,ST.ST_Id,AL.ALC_Id,PL.PL_Name, PL.PL_Id,SUM(AB.ABP_AmountRecieved) AS ABP_AmountRecieved ";
...
if($filterData[7] != '' && $filterData[8] != '') {   // after headers are loaded
    ...
    if($filterData[7] == 'a' || $filterData[7] == 'b') {
        if($filterData[7] == 'a' && $ascFlter == "DESC")
            $slNo = 2;              // descending sl no:
        
        $flt    .=  " ORDER BY PL.PL_Name  $ascFlter";
        ...
        $flt    .= " ORDER BY ".$monthArray[$filterData[7]]."  ".$ascFlter;
    } else {
...
$AtObj->reportPlaceData($fields,$filter,$REQUEST['f'],$REQUEST['t'],$_GET["posStart"],$_GET["count"],$flt);        
$monthArray             = array("","January", "February", "March","April", "May", "June","July", "August", "September","October", "November", "December");
$Place_Obj               = $AtObj->PlaceReportArray;

$AtObj->getPlaceFooter($filter,$REQUEST['f'],$REQUEST['t']);
$Count                  = $AtObj->reportPlaceDataCount($filter,$REQUEST['f'],$REQUEST['t']);
...
echo '<rows total_count="'.$Count.'" pos="'.$_GET["posStart"].'">
    ...
    if (($_GET["posStart"] == 0 && !isset($flag))) {
        ...
        <column width="150" sort = "na"  type="ro" align="left" ><![CDATA[<div><div id="placeFlt" style="width: 70%;float:left;" placeholder="Place"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="b" class="tkPL_Sort_BIR"/></div></div>]]></column>';
        $colNum     = 2;
            ...
            echo '<row id = "'.$rw->PL_Id.'">
                ...
                for($y = $toYear; $y >= $fromYear; $y--) {
                    ...
                    for($i = $monthLimit; $i >= $month; $i--) {
                        ...
                        echo $amount       = ($AtObj->PlaceMonthReportArray[$rw->PL_Id][$y][$monthLimit]) ? $AtObj->PlaceMonthReportArray[$rw->PL_Id][$y][$monthLimit] : 0;
