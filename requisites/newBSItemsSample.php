...
$itemSearch="AND 1";
$itemSearch2="AND 1";
$orderBy="ORDER BY IT.IT_Name ";
if($REQUEST["mask"]){
$itemSearch="AND IT.IT_Name  REGEXP '[[:<:]]".$REQUEST["mask"]."'";
$orderBy="ORDER BY CASE WHEN IT.IT_Name like '".$REQUEST["mask"]."%' THEN 0
               WHEN IT.IT_Name REGEXP '[[:<:]]".$REQUEST["mask"]."' THEN 1               
               ELSE 3 END, IT.IT_Name";
//$orderBy=" ORDER BY IT.IT_Name";
}
$type = $REQUEST['type'];
...
$ItemObj->getItemList("IT.IT_Id, IT.IT_Name","as IT, main_heads as MH, sub_heads as SH ","  WHERE ( IT.IT_Status = 1 || IT.IT_Status = 3 ) AND IT.SH_Id=SH.SH_Id AND SH.MH_Id=MH.MH_Id AND MH.MH_Type = ".$type." AND (IT.OF_Id=".$preTally_user_ofid." ".$itemSearch." OR IT.IT_Id IN (".$itemMap."))".$orderBy);
