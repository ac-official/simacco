...
$NotificationObj->viewCashPaymentNoft($filter, $filter_mask,$_GET["posStart"],$_GET["count"]);
$Count = $NotificationObj->viewCashPaymentNoftCount($filter, $filter_mask);
...
//            $expense = ($rw->MH_Type == '2') ? $amount : "---";
            echo '
                <row id="'.$rw->BS_Id.'">
                    ...
                    <cell>'.$amount.'</cell>
                    <cell><![CDATA[<a style="text-decoration:none;" class="targetCash_'.$rw->BS_Id.'" onclick="preTally.BankBalanceSheet.showDetailData(this,'.$rw->BS_Id.','.$rw->SH_Id.');">'.$rw->LC_Name.'</a>]]></cell>';
                    if(($blockedDate < $rw->BS_Date)) {
                        echo '<cell><![CDATA[<a href="#" style="text-decoration:none;" onclick="preTally.Notification.confirmCashPaymentNotf(this,'.$rw->BS_Id.','.$rw->IT_DualItem.');"><span class="confirmBtn" >Confirm Payment</span></a>]]></cell>';
                    } else {
                        echo '<cell></cell>';    
                    }
                echo '</row>';
