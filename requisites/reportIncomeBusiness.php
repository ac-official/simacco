...
$filter     = ' IT.MH_Type =1 AND IT.IT_Business = 1 AND BS.TR_Id != "NULL" AND  BS.TR_Id != "0"  AND LC.OF_Id = '.$preTally_user_ofid.' AND BS.BS_Status = 1 ';
$filt       = 'LC.OF_Id = '.$preTally_user_ofid;

if(isset($REQUEST['LC_Id']) && is_numeric($REQUEST['LC_Id']))
    $filt  .= ' AND LC.LC_Id = '.$REQUEST['LC_Id'];
...
if($fromYear == $toYear) {
    $oldStockFrom   = '01-04-'.($fromYear-1);
    $oldStockTo     = '31-03-'.($toYear);
} else {
    $oldStockFrom   = '01-04-'.($fromYear-1);
    $oldStockTo     = '31-03-'.($toYear-1);
}
$fromMonth          = ltrim($fromDateElements[1],0);
$toMonth            = ltrim($toDateElements[1],0);

$BSRptObj->reportBusinessData($filt,$filter,$REQUEST['f'],$REQUEST['t'],$oldStockFrom,$oldStockTo);
...
echo("<?xml version='1.0' encoding='iso-8859-1'?>\n");
    echo '<rows>';
    echo '<head>
        <column width="50"  type="ro" align="center" ><![CDATA[<div style="text-align:left;"> SlNo <img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="0" class="inc_bs_Sort_BIR"/></div>]]></column>
        <column width="120"  type="ro" align="left" ><![CDATA[<div><div id="bs_branchItmF" style="width: 70%;float:left;" placeholder="Branch"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="1" class="inc_bs_Sort_BIR"/></div></div>]]></column>';
    if($MRP_Obj && $BSRptObj->BusinessArray) {
        ...
        for($y = $toYear; $y >= $fromYear; $y--) {
            ...
            for($i = $monthLimit; $i >= $month; $i--) {
                ...
                echo '<column width="*" format = "0,000" type="ron" align="right" ><![CDATA[<div style="float:left;">'.$monthArray[$monthLimit] .'<br>'.$y .'<img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="'.$colNum.'" class="inc_bs_Sort_BIR"/> </div>]]></column>';
                $colNum++;
                $monthLimit--;
                $footer         .= ",{#stat_total}";
        ...
        echo '<column width="140" format = "0,000" type="ron" align="right" ><![CDATA[<div style="float:left;"> Total <img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="'.$colNum.'" class="inc_bs_Sort_BIR" /> </div>]]></column>
            <column width="140" format = "0,000" type="ron" align="right" ><![CDATA[<div style="float:left;"> Old Stock <img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="'.($colNum+1).'" class="inc_bs_Sort_BIR" /> </div>]]></column>
            <afterInit>
                <call command="enableTooltips"><param>false,false'.$toolTip.',false,false</param></call>
                <call command="setColumnMinWidth"><param>,'.$cellFormat.',,</param></call> 
                <call command="attachFooter"><param>Total,#cspan'.$footer.',{#stat_total},{#stat_total}</param>
        ...
        foreach($MRP_Obj as $rw) {
            ...
            echo '<row id = "'.$rw->LC_Id.'">
                    ...
                    for($y = $toYear; $y >= $fromYear; $y--) {
                        ...
                        $arr_amts   =  arrayFlatten($BSRptObj->BusinessArray[$rw->LC_Id]);                        
                        $maxamt     =  0;
                        $maxamt     =  max($arr_amts);
                        for($i = $monthLimit; $i >= $month; $i--) {
                            if($monthLimit == 0) 
                            ...
                            if($maxamt == $BSRptObj->BusinessArray[$rw->LC_Id][$y][$i] && $maxamt > 0)
                                $bgcolor = 'style="background-color:#33cc33;"';
                            else
                                $bgcolor = ''; 
                            echo '<cell name="month'.$y.$i.'" title= " " '.$bgcolor.'>';
                            echo $business = ($BSRptObj->BusinessArray[$rw->LC_Id][$y][$monthLimit]) ? round($BSRptObj->BusinessArray[$rw->LC_Id][$y][$monthLimit]) : 0;
                    ...
                    <cell name="OldStock" title= " ">';
                    echo $oldStock = $BSRptObj->StockArray[$rw->LC_Id]['CurrentPeriodOldStock'] + $BSRptObj->StockArray[$rw->LC_Id]['Old_Business'] - $BSRptObj->StockArray[$rw->LC_Id]['Old_Income'];
                    echo '</cell>
            </row>';
            $j++;
        }
