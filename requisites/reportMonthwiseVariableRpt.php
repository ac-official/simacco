...
echo("<?xml version='1.0' encoding='iso-8859-1'?>\n");
    echo '<rows>';
    echo '<head>
        <column width="50"  type="ro" align="center" ><![CDATA[<div style="text-align:left;"> SlNo <img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="0" class="var_Sort_BIR"/></div>]]></column>
        <column width="120"  type="ro" align="left" ><![CDATA[<div><div id="varBranchStatItmF" style="width: 70%;float:left;" placeholder="Branch"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="1" class="var_Sort_BIR"/></div></div>]]></column>';
    if($MRP_Obj && $BSRptObj->VarMonthReportArray) {
        ...
        for($y = $toYear; $y >= $fromYear; $y--) {
            ...
            for($i = $monthLimit; $i >= $month; $i--) {
                ...
                echo '<column width="*" format = "0,000" type="ron" align="right" ><![CDATA[<div style="float:left;">'.$monthArray[$monthLimit] .'<br>'.$y .'<img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="'.$colNum.'" class="var_Sort_BIR"/> </div>]]></column>';
                $colNum++;
                $monthLimit--;
                $footer         .= ",{#stat_total}";
        ...
        echo '<column width="140" format = "0,000" type="ron" align="right" ><![CDATA[<div style="float:left;"> Total <img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="'.$colNum.'" class="var_Sort_BIR" /> </div>]]></column>
            <afterInit>
            ...
            echo '<row id = "'.$rw->LC_Id.'">
                    ...
                    for($y = $toYear; $y >= $fromYear; $y--) {
                        ...
                        for($i = $monthLimit; $i >= $month; $i--) {
                            ...
                            echo $amount = ($BSRptObj->VarMonthReportArray[$rw->LC_Id][$y][$monthLimit]) ? $BSRptObj->VarMonthReportArray[$rw->LC_Id][$y][$monthLimit] : '0';
                            $totAmount += $amount;
                            echo '</cell>';
                            $monthLimit--;
                        }
