...
$filter     = ' AJ.AJ_Status != 0 AND AJ.AJ_Status != 2 AND AJ.OF_Id = '.$preTally_user_ofid;
$fields     = " CT.ST_Id,CT.CT_Name, CT.CT_Id,SUM(AB.ABP_AmountRecieved) AS ABP_AmountRecieved  ";
...
if($filterData[5] != '' && $filterData[6] != '') {   // after headers are loaded
    ...
    if($filterData[5] == 'a' || $filterData[5] == 'b') {
        if($filterData[5] == 1 && $ascFlter == "DESC")
            $slNo = 2;              // descending sl no:
        
        $flt    .=  " ORDER BY CT.CT_Name $ascFlter";
        ...
        $flt .= " ORDER BY ".$monthArray[$filterData[5]]." ".$ascFlter;
    } else {
...
$AtObj->reportCityData($fields,$filter,$REQUEST['f'],$REQUEST['t'],$_GET["posStart"],$_GET["count"],$flt);        
$City_Obj               = $AtObj->CityReportArray;

$AtObj->getCityFooter($filter,$REQUEST['f'],$REQUEST['t']);
$Count      = $AtObj->reportCityDataCount($filter,$REQUEST['f'],$REQUEST['t']);
...
echo '<rows total_count="'.$Count.'" pos="'.$_GET["posStart"].'">
    ...
    if (($_GET["posStart"] == 0 && !isset($flag))) {
        ...
        <column width="150"  type="ro" align="left" ><![CDATA[<div><div id="cityFlt" style="width: 70%;float:left;" placeholder="City"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="b" class="tkCt_Sort_BIR"/></div></div>]]></column>';
            ...
            echo '<row id = "'.$rw->CT_Id.'">
                ...
                for($y = $toYear; $y >= $fromYear; $y--) {
                    ...
                    for($i = $monthLimit; $i >= $month; $i--) {
                        ...
                        echo $amount = ($AtObj->CityMonthReportArray[$rw->CT_Id][$y][$monthLimit]) ? $AtObj->CityMonthReportArray[$rw->CT_Id][$y][$monthLimit] : 0;
