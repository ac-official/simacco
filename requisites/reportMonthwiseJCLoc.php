...
if($filterData[6] != '' && $filterData[7] != '') {   // after headers are loaded
    ...
    } else if($filterData[6] != 13 ) { //check filter month  js-filtermonth
...
$AtObj->reportJobCountLocationData($filter,$REQUEST['f'],$REQUEST['t'],$flt,$orderFilter,$fields,$statusFilter);        
$Loc_Obj                = $AtObj->JobCountLocationReportArray;
$AtObj->getJobLocFooter($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter);
$Count                  = $AtObj->reportJobLocationDataCount($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter);
...
echo '<rows total_count="'.$Count.'" pos="'.$_GET["posStart"].'">
    ...
    if (($_GET["posStart"] == 0 && !isset($flag))) {
        ...
        <column width="150" sort = "na"  type="ro" align="left" ><![CDATA[<div><div id="JloctnFlt" style="width: 70%;float:left;" placeholder="City"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="b" class="tkLC_Sort_BIR"/></div></div>]]></column>';
        $colNum     = 2;
            ...
            echo '<row id = "'.$rw->ALC_Id.'">
                ...
                for($y = $toYear; $y >= $fromYear; $y--) {
                    ...
                    for($i = $monthLimit; $i >= $month; $i--) {
                        ...
                        echo $amount       = ($AtObj->JLocMonthReportArray[$rw->ALC_Id][$y][$monthLimit]) ? $AtObj->JLocMonthReportArray[$rw->ALC_Id][$y][$monthLimit] : 0;
