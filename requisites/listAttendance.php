...
$month = date('M');
if ($stdate != "" && $lastdate != "") {
    $filter = " WHERE AT_Date BETWEEN '" . $stdate . "' AND '" . $lastdate . "' AND US_Id=" . $preTally_user_id ." ORDER BY AT_Date DESC";
    $month = date("M", strtotime($stdate));
} else {
    $month == date("M");
    $lastdate = date("Y-m-t", strtotime("-1 " . $month));
    $stdate = date("Y-m-1", strtotime("-1 " . $month));    
    $filter = " WHERE AT_Date BETWEEN '" . $stdate . "' AND '" . $lastdate . "' AND US_Id=" . $preTally_user_id ." ORDER BY AT_Date DESC";
...
$att_res = $AttObj->calcAttendance($month, $curDate, $preTally_user_id);
...
if ($Att_Obj) {
    ...
    foreach ($Att_Obj as $rw) {
        ...
        $statLabel = "Unmarked";
          ...
          if (($date != date("Y-m-d")) && ($rw->AT_Status == 0 )) {
                $statLabel = "Invalid";
