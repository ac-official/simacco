...
$reportItems = array(
                    ...
                    'entry'      => array('0','All Entries','entry.gif')
                );
...
echo '<rows parent=\''.$reqID.'\'>';
    ...
    if(is_array($IDArray))$request = $IDArray['request'];
    else $request = $IDArray['request'] = 'allYear' ;
...
function yearWise() {
    ...
    for($year = $toYear; $year > $fromYear; $year--) {
        ...
        $result .= '<row id=\''.json_encode($nodeID).'\' xmlkids="1">
                        <cell style="">'.($year-1).' - '.($year).' -> Month</cell>
                            '.$MISData.'
                    </row>';
    }
    return $result;
...
//        $endDate        = new DateTime($thisYear[0].'-04-01');
    ...
    while ($firstDate <> $lastDate) {
        ...
        $MISData[$month]['BusInc']      = $MISData[$month]['BusInc'] !='' ? $MISData[$month]['BusInc'].$currency  : $nullValue ;
        $MISData[$month]['BusExp']      = $MISData[$month]['BusExp'] !='' ? $MISData[$month]['BusExp'].$currency  : $nullValue ;
        $MISData[$month]['OtrInc']      = $MISData[$month]['OtrInc'] !='' ? $MISData[$month]['OtrInc'].$currency  : $nullValue ;
        $MISData[$month]['OtrExp']      = $MISData[$month]['OtrExp'] !='' ? $MISData[$month]['OtrExp'].$currency  : $nullValue ;
        $MISData[$month]['CshTransRecv']  = $MISData[$month]['CshTransRecv'] !='' ? $MISData[$month]['CshTransRecv'].$currency  : $nullValue ;
        $MISData[$month]['CshTransPaid']  = $MISData[$month]['CshTransPaid'] !='' ? $MISData[$month]['CshTransPaid'].$currency  : $nullValue ;
        ...
        $result .= '<row id=\''.json_encode($nodeID).'\' xmlkids="1">
                        <cell style="">'.$firstDate->format('F - Y').' -> Date</cell>
...
//    for($tempDay = 31; $tempDay >= 1; $tempDay --) {
      while($tempDay >= 1 || $tempDay <= 31){  
        ...
        if (date('m', $time) == $thisMonth) {     
            ...
            $MISData[$day]['BusInc']      = $MISData[$day]['BusInc'] !='' ? $MISData[$day]['BusInc'].$currency  : $nullValue ;
            $MISData[$day]['BusExp']      = $MISData[$day]['BusExp'] !='' ? $MISData[$day]['BusExp'].$currency  : $nullValue ;
            $MISData[$day]['OtrInc']      = $MISData[$day]['OtrInc'] !='' ? $MISData[$day]['OtrInc'].$currency  : $nullValue ;
            $MISData[$day]['OtrExp']      = $MISData[$day]['OtrExp'] !='' ? $MISData[$day]['OtrExp'].$currency  : $nullValue ;
            ...
            $result .= '<row id=\''.json_encode($nodeID).'\' xmlkids="1">
                            <cell style="">'.date("dS M Y (l)", $time).' -> Items</cell>
...
function branchWise ($reqArray) {
    ...
    foreach ($MISData as $key => $value) {
        ...
        $result .= '<row id=\''.json_encode($nodeID).'\' xmlkids="1">
                        <cell style="">'.$value['LCName'].' -> Users</cell>
...
function itemWise ($reqArray) {
    ...
    foreach ($MISData as $key => $value) {
        ...
        $result .= '<row id=\''.json_encode($nodeID).'\' xmlkids="1">
                        <cell style="">'.$value['ITName'].' -> Branches</cell>
                        <cell>'.$value['BusInc'].'</cell>
                        <cell>'.$value['BusExp'].'</cell>
                        <cell>'.$BusinessPL.'</cell>
                        <cell>'.$value['OtrInc'].'</cell>
...
function userWise ($reqArray) {
    ...
    foreach ($MISData as $key => $value) {
        ...
        $nodeID['user']    = array($value['USId'],$value['USName'],'user.gif');
    ...
    foreach ($reportItems as $key => $value) {
        ...
        if (!array_key_exists($key, $nodeID) && $mainNode !== $key && strtolower(str_replace("all","",$requestString)) !== $key) {    
            ...
            if($continue) {
                ...
                $nodeID['seed']    = makeSeed();
                $result .= '<row id=\''.json_encode($nodeID).'\' xmlkids="1"  style="">
                                <cell style=""><![CDATA[<b style="">'.$value[1].'</b>]]></cell>
                            </row>';
            }
...
//            $lastDate = date('t',strtotime($sDate));
    ...
    if($nodeID['request'] == 'allMonth') {
        ...
        if($beginDate && $endDate){
            ...
            $RptData['BusInc']      = $RptData['BusInc'] !='' ? $RptData['BusInc'].$currency  : $nullValue ;
            $RptData['BusExp']      = $RptData['BusExp'] !='' ? $RptData['BusExp'].$currency  : $nullValue ;
            $RptData['OtrInc']      = $RptData['OtrInc'] !='' ? $RptData['OtrInc'].$currency  : $nullValue ;
            $RptData['OtrExp']      = $RptData['OtrExp'] !='' ? $RptData['OtrExp'].$currency  : $nullValue ;
            $RptData['CshTransRecv']  = $RptData['CshTransRecv'] !='' ? $RptData['CshTransRecv'].$currency  : $nullValue ;
            $RptData['CshTransPaid']  = $RptData['CshTransPaid'] !='' ? $RptData['CshTransPaid'].$currency  : $nullValue ;
            $BusinessPL = $BusinessPL !='' ? $BusinessPL.$currency  : $nullValue ;
            $TotalInc   = $TotalInc   !='' ? $TotalInc.$currency    : $nullValue ;
            $TotalExp   = $TotalExp   !='' ? $TotalExp.$currency    : $nullValue ;
