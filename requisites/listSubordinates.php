...
$UsrObj->userReportsMe(' DD.US_Id, DD.US_FName AS DD_FName, DD.US_LName AS DD_LName ', ' AS DD WHERE US_Id IN ('.$Rprtid.') AND DD.US_Status=1 AND DD.US_ResignFlag=0 ORDER BY DD_FName' );
$US_List = $UsrObj->UserArray;
echo("<?xml version='1.0' encoding='iso-8859-1'?>\n");
echo '<complete>';   
if($US_List){ 
    echo "<option selected='true'>Select Subordinate</option>";
    foreach($US_List as $rw){  
        if($preTally_user_id!=$rw->US_Id){
