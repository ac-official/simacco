...
if($filterData[2] && is_numeric($filterData[2])) {
    $filter     .= ' AND AJDT.CT_Id = '.$filterData[2]; 
}
...
if($filterData[6] != '' && $filterData[7] != '') {   // after headers are loaded
    ...
    if($filterData[6] == 'a' || $filterData[6] == 'b') {
        if($filterData[6] == 'a' && $orderBy == "DESC")
            $slNo = 2;              // descending sl no:
        
        $flt    .=  " ORDER BY AL.ALC_Name $orderBy";
        $orderFilter    .=  " , AL.ALC_Name $orderBy";
        ...
        $fields .= " ,SUM(CASE WHEN MONTH(AJDT.AJ_ReceivedDate) = 1 THEN 1 ELSE 0 END) AS January,
         ...
         $flt .= " ORDER BY ".$monthArray[$filterData[6]]." ".$orderBy;
...
$AtObj->reportDocumentCountLocationData($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter,$flt,$orderFilter,$fields);        
$Loc_Obj                = $AtObj->DocumentCountLocationReportArray;
$AtObj->getDocumentLocFooter($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter);
$Count                  = $AtObj->reportDocumentLocationDataCount($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter,$flt);
...
echo '<rows total_count="'.$Count.'" pos="'.$_GET["posStart"].'">
    ...
    if (($_GET["posStart"] == 0 && !isset($flag))) {
        ...
        <column width="150" sort = "na"  type="ro" align="left" ><![CDATA[<div><div id="CloctnFlt" style="width: 70%;float:left;" placeholder="City"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="b" class="tkLC_Sort_BIR"/></div></div>]]></column>';
        $colNum     = 2;
            ...
            echo '<row id = "'.$rw->ALC_Id.'">
                ...
                for($y = $toYear; $y >= $fromYear; $y--) {
                    ...
                    for($i = $monthLimit; $i >= $month; $i--) {
                        ...
                        echo $amount       = ($AtObj->DELocationMonthReportArray[$rw->ALC_Id][$y][$monthLimit]) ? $AtObj->DELocationMonthReportArray[$rw->ALC_Id][$y][$monthLimit] : 0;
