...
if($filterData[8] != '' && $filterData[9] != '') {   // after headers are loaded
    ...
    } else if($filterData[8] != 13 ) { //check filter month  js-filtermonth
...
$AtObj->reportEnquiryCountStreetData($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter,$flt,$orderFilter,$fields );        
$Street_Obj                = $AtObj->EnquiryCountStreetReportArray;
$AtObj->getEnquiryStreetFooter($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter);
$Count                  = $AtObj->reportEnquiryStreetDataCount($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter);
...
echo '<rows total_count="'.$Count.'" pos="'.$_GET["posStart"].'">
    ...
    if (($_GET["posStart"] == 0 && !isset($flag))) {
        ...
        <column width="150" sort = "na"  type="ro" align="left" ><![CDATA[<div><div id="EstreetFlt" style="width: 70%;float:left;" placeholder="Place"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="b" class="tkSTR_Sort_BIR"/></div></div>]]></column>';
        $colNum     = 2;
            ...
            echo '<row id = "'.$rw->SR_Id.'">
                ...
                for($y = $toYear; $y >= $fromYear; $y--) {
                    ...
                    for($i = $monthLimit; $i >= $month; $i--) {
                        ...
                        echo $amount       = ($AtObj->EStreetMonthReportArray[$rw->SR_Id][$y][$monthLimit]) ? $AtObj->EStreetMonthReportArray[$rw->SR_Id][$y][$monthLimit] : 0;
