...
$AtObj->reportJobCountPlaceData($filter,$REQUEST['f'],$REQUEST['t'],$flt,$orderFilter,$fields,$statusFilter);        
$Place_Obj                = $AtObj->JobCountPlaceReportArray;
$AtObj->getJobPlaceFooter($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter);
$Count                  = $AtObj->reportJobPlaceDataCount($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter);
...
echo '<rows total_count="'.$Count.'" pos="'.$_GET["posStart"].'">
    ...
    if (($_GET["posStart"] == 0 && !isset($flag))) {
        ...
        <column width="150" sort = "na"  type="ro" align="left" ><![CDATA[<div><div id="JplaceFlt" style="width: 70%;float:left;" placeholder="Place"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="b" class="tkPL_Sort_BIR"/></div></div>]]></column>';
        $colNum     = 2;
            ...
            echo '<row id = "'.$rw->PL_Id.'">
                ...
                for($y = $toYear; $y >= $fromYear; $y--) {
                    ...
                    for($i = $monthLimit; $i >= $month; $i--) {
                        ...
                        echo $amount       = ($AtObj->JPlaceMonthReportArray[$rw->PL_Id][$y][$monthLimit]) ? $AtObj->JPlaceMonthReportArray[$rw->PL_Id][$y][$monthLimit] : 0;
