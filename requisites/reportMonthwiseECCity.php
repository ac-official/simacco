...
$year       = ($yrFilter) ? $yrFilter : date("Y");
if($REQUEST['m']) {   // month is selected
    /*if((date('n') >= 4)&&(date('n') <= 9)) {  // current financial year
        $fromYear   = date('Y')-1;
        $toYear     = ($mntFilter <= 3) ? date('Y')-1 : date('Y');
        $REQUEST['f']   = '01-10-'.$fromYear;
    } else if(date('n') <= 3) {
        $fromYear   = date('Y')-1;
        $toYear     = ($mntFilter >= 4) ? date('Y')-1 : date('Y');
        $REQUEST['f']   =  '01-'.($mntFilter+6).'-'.$fromYear;
    } else if(date('n') >= 10) {
                $fromYear   = date('Y');
        $toYear     = ($mntFilter >= 4) ? date('Y') : date('Y')-1;
        $frmMonth    = ($mntFilter > 6) ? ($mntFilter-6) : ($REQUEST['m']+6);
        if($mntFilter > 6)$frmMonth=$mntFilter-6;
        else{
            $fromYear   = date('Y')-1; 
            $frmMonth   = $mntFilter+6;            
        } 
...
$AtObj->reportEnquiryCountCityData($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter,$flt,$orderFilter,$fields);        
$City_Obj     = $AtObj->EnquiryCountCityReportArray;
$CTObj        = $AtObj->ECityMonthReportArray;
$Count      = $AtObj->reportEnquiryCityDataCount($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter);

$AtObj->getEnquiryCityFooter($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter);
...
echo '<rows total_count="'.$Count.'" pos="'.$_GET["posStart"].'">
    ...
    if (($_GET["posStart"] == 0 && !isset($flag))) {
        ...
        <column width="150"  type="ro" align="left" ><![CDATA[<div><div id="EcityFlt" style="width: 70%;float:left;" placeholder="City"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="b" class="tkCt_Sort_BIR"/></div></div>]]></column>';
            ...
            echo '<row id = "'.$rw->CT_Id.'">
                ...
                for($y = $toYear; $y >= $fromYear; $y--) {
                    ...
                    for($i = $monthLimit; $i >= $month; $i--) {
                        ...
                        echo $amount = ($AtObj->ECityMonthReportArray[$rw->CT_Id][$y][$monthLimit]) ? $AtObj->ECityMonthReportArray[$rw->CT_Id][$y][$monthLimit] : 0;
