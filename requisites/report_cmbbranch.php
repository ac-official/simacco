...
$typeOfEntryFiter='';
 if($REQUEST['typeOfEntry']){
    ...
    $UserObj->getBranchFilterCombo($REQUEST['f'],$REQUEST['t'],$filter);
    $US_Obj = $UserObj->BranchFitlerData;
...
echo '<option value="0" '.$default_select.'>Select Branch</option>';	
if($US_Obj){
    foreach($US_Obj as $rw){
        if($LCId && $rw->LC_Id == $LCId) { $selected = 'selected = "'. 1 .'" ' ; } 
        echo '<option value="'.$rw->LC_Id.'" '.$selected.'>'.$rw->LC_Name.'</option>';
    }
}
echo '</complete>';
?>