...
$BSRptObj->reportFixedExpenseData($filt,$filter,$REQUEST['f'],$REQUEST['t'],$preTally_user_ofid);
...
echo("<?xml version='1.0' encoding='iso-8859-1'?>\n");
    ...
    echo '<head>
        ...
        <column width="120"  type="ro" align="left" ><![CDATA[<div><div id="fxdBranchItmF" style="width: 70%;float:left;" placeholder="Branch"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="1" class="bs_Sort_BIR"/></div></div>]]></column>';
    if($MRP_Obj && $BSRptObj->FixedMonthReportArray) {
        ...
        foreach($MRP_Obj as $rw) {
            ...
            echo '<row id = "'.$rw->LC_Id.'">
                    ...
                    for($y = $toYear; $y >= $fromYear; $y--) {
                        ...
                        $arr_amts   =  arrayFlatten($BSRptObj->FixedMonthReportArray[$rw->LC_Id]);                        
                        $maxamt     =  0;
                        $maxamt     =  max($arr_amts);
                        for($i = $monthLimit; $i >= $month; $i--) {
                            if($monthLimit == 0) 
                            ...
                            if($maxamt == $BSRptObj->FixedMonthReportArray[$rw->LC_Id][$y][$i] && $maxamt > 0)
                                $bgcolor = 'style="background-color:#33cc33;"';
                            else
                                $bgcolor = ''; 
                            
                            echo '<cell name="month'.$y.$i.'" title= " " '.$bgcolor.'>';
                            echo $amount = ($BSRptObj->FixedMonthReportArray[$rw->LC_Id][$y][$monthLimit]) ? round($BSRptObj->FixedMonthReportArray[$rw->LC_Id][$y][$monthLimit]) : '0';
                            $totAmount += $amount;
                            echo '</cell>';
                            $monthLimit--;
                        }
