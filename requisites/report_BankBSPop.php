...
/*if($REQUEST['SHID']!=58 && $REQUEST['SHID']!=59){*/
    ...
    // End 02-06-2025
        ...
        $formData .= '{type: "fieldset", name:"fieldsetname", class: "mydata",  label: "<img src=\'images/icon/close_button_icon.png\' style=\'cursor:pointer;\' onclick=\'preTally.BankBalanceSheet.hideDetailData()\' />", width:"auto", list:[';
        ...
        if($msgObj->MsgString)$fieldCount=$fieldCount+1;
        foreach ($curField as $value) {            
            ...
            if($value == "BS_PaidDate") 
                $Combo_Name = date('d/m/Y',  strtotime($BS_Obj[0]->BS_Date)); 
        ...
        //$formData .=']}';        
        if($msgObj->MsgString){$formData .= ' { type : "template", name : "MSGS", label : "Messages: ", value : " <b>'.wordwrap(str_replace("\"", "",html_entity_decode($msgObj->MsgString)),50,"<br>",TRUE).'</b>" }';}
    
    $formData .= ']}]';
    echo $formData;
/*} else {
    $formData1 = '[{type: "settings",position: "label-left", labelWidth: "190", inputWidth: "auto"},';   
    $formData1 .= '{type: "fieldset", name:"fieldsetname", label: "<img src=\'images/icon/close_button_icon.png\' style=\'cursor:pointer;\' onclick=\'preTally.BankBalanceSheet.hideDetailData()\' />", width:"auto", list:[';
        $formData1 .= ' { type : "template" , label : "<b>Waiting for admin Approval.</b>" , value : "<b> Contact your administrator for further information.</b> " },';
        $formData1 .= '{ type:"newcolumn"},'; 
