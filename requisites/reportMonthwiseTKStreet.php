...
$filter         = ' SR.SR_Status != 0 AND AJ.AJ_Status != 0 AND AJ.AJ_Status != 2 AND AJ.OF_Id = '.$preTally_user_ofid;
$fields         = " SR.SR_Id,SR.SR_Name,CT.CT_Id,ST.ST_Id,AL.ALC_Id,PL.PL_Id,SUM(AB.ABP_AmountRecieved) AS ABP_AmountRecieved ";
...
if($filterData[8] != '' && $filterData[9] != '') {   // after headers are loaded
    ...
    if($filterData[8] == 'a' || $filterData[8] == 'b') {
        if($filterData[8] == 'a' && $ascFlter == "DESC")
            $slNo = 2;              // descending sl no:
        
        $flt    .=  " ORDER BY SR.SR_Name $ascFlter";
        ...
        $flt    .= " ORDER BY ".$monthArray[$filterData[8]]." ".$ascFlter;
    } else {
...
$AtObj->reportStreetData($fields,$filter,$REQUEST['f'],$REQUEST['t'],$_GET["posStart"],$_GET["count"],$flt);        
$monthArray             = array("","January", "February", "March","April", "May", "June","July", "August", "September","October", "November", "December");
$Street_Obj             = $AtObj->StreetReportArray;

$AtObj->getStreetFooter($filter,$REQUEST['f'],$REQUEST['t']);
$Count                  = $AtObj->reportStreetDataCount($filter,$REQUEST['f'],$REQUEST['t']);
...
echo '<rows total_count="'.$Count.'" pos="'.$_GET["posStart"].'">
    ...
    if (($_GET["posStart"] == 0 && !isset($flag))) {
        ...
        <column width="150" sort = "na"  type="ro" align="left" ><![CDATA[<div><div id="streetFlt" style="width: 70%;float:left;" placeholder="Place"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="b" class="tkSTR_Sort_BIR"/></div></div>]]></column>';
        $colNum     = 2;
            ...
            echo '<row id = "'.$rw->SR_Id.'">
                ...
                for($y = $toYear; $y >= $fromYear; $y--) {
                    ...
                    for($i = $monthLimit; $i >= $month; $i--) {
                        ...
                        echo $amount       = ($AtObj->StreetMonthReportArray[$rw->SR_Id][$y][$monthLimit]) ? $AtObj->StreetMonthReportArray[$rw->SR_Id][$y][$monthLimit] : 0;
