...
//$AtObj->reportStateData($filter,$REQUEST['f'],$REQUEST['t']);        
//$City_Obj               = $AtObj->StateReportArray;
//
//$AtObj->getStateFooter($filter,$REQUEST['f'],$REQUEST['t']);


$AtObj->reportDocumentCountStateData($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter);
$City_Obj    = $AtObj->DocumentCountReportArray;

$AtObj->getDocumentStateFooter($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter);
...
echo '<rows>';
    ...
    <column width="150"  sort = "na" type="ro" align="left" ><![CDATA[<div><div id="CstateFlt" style="width: 70%;float:left;" placeholder="City"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="1" class="tkSt_Sort_BIR"/></div></div>]]></column>';
            ...
            echo '<row id = "'.$rw->ST_Id.'">
                ...
                for($y = $toYear; $y >= $fromYear; $y--) {
                    ...
                    for($i = $monthLimit; $i >= $month; $i--) {
                        ...
                        echo $amount        = ($AtObj->DStateMonthReportArray[$rw->ST_Id][$y][$monthLimit]) ? $AtObj->DStateMonthReportArray[$rw->ST_Id][$y][$monthLimit] : 0;
