...
$AtObj->reportDocumentCountPlaceData($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter,$flt,$orderFilter,$fields);        
$Place_Obj                = $AtObj->DocumentCountPlaceReportArray;
$AtObj->getDocumentPlaceFooter($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter);
$Count                  = $AtObj->reportDocumentPlaceDataCount($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter);
...
echo '<rows total_count="'.$Count.'" pos="'.$_GET["posStart"].'">
    ...
    if (($_GET["posStart"] == 0 && !isset($flag))) {
        ...
        <column width="150" sort = "na"  type="ro" align="left" ><![CDATA[<div><div id="CplaceFlt" style="width: 70%;float:left;" placeholder="Place"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="b" class="tkPL_Sort_BIR"/></div></div>]]></column>';
        $colNum     = 2;
            ...
            echo '<row id = "'.$rw->PL_Id.'">
                ...
                for($y = $toYear; $y >= $fromYear; $y--) {
                    ...
                    for($i = $monthLimit; $i >= $month; $i--) {
                        ...
                        echo $amount       = ($AtObj->DPlaceMonthReportArray[$rw->PL_Id][$y][$monthLimit]) ? $AtObj->DPlaceMonthReportArray[$rw->PL_Id][$y][$monthLimit] : 0;
