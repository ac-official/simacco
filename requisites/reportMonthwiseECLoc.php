...
if($filterData[2] && is_numeric($filterData[2])) {
    $filter     .= ' AND AE.CT_Id = '.$filterData[2]; 
}
...
$AtObj->reportEnquiryCountLocationData($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter,$flt,$orderFilter,$fields);        
$Loc_Obj                = $AtObj->EnquiryCountLocationReportArray;
$AtObj->getEnquiryLocFooter($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter);
$Count                  = $AtObj->reportEnquiryLocationDataCount($filter,$REQUEST['f'],$REQUEST['t'],$statusFilter,$flt);
...
echo '<rows total_count="'.$Count.'" pos="'.$_GET["posStart"].'">
    ...
    if (($_GET["posStart"] == 0 && !isset($flag))) {
        ...
        <column width="150" sort = "na"  type="ro" align="left" ><![CDATA[<div><div id="EloctnFlt" style="width: 70%;float:left;" placeholder="City"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="b" class="tkLC_Sort_BIR"/></div></div>]]></column>';
        $colNum     = 2;
            ...
            echo '<row id = "'.$rw->ALC_Id.'">
                ...
                for($y = $toYear; $y >= $fromYear; $y--) {
                    ...
                    for($i = $monthLimit; $i >= $month; $i--) {
                        ...
                        echo $amount       = ($AtObj->ELocationMonthReportArray[$rw->ALC_Id][$y][$monthLimit]) ? $AtObj->ELocationMonthReportArray[$rw->ALC_Id][$y][$monthLimit] : 0;
