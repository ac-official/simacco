<?php
/**
* Accounts Vendors Listing with filter and pagination
...
$inparams['gstno']		= (isset($filterData[1])) ? (string)$filterData[1] : "";
$inparams['contacts']		= (isset($filterData[2])) ? (string)$filterData[2] : "";
$inparams['status']		= (isset($filterData[3])) ? (string)$filterData[3] : "";
...
echo '<rows total_count="'.$total_records.'" pos="'.$inparams['start'].'">
      ...
      if (!empty($result_data)) {
		...
		foreach($result_data as $rw) {
			...
			.'<userdata name="gst_no">'.$rw->gst_no.'</userdata>'
			.'<userdata name="type">'.$rw->type.'</userdata>'
			.'<userdata name="email">'.$rw->email.'</userdata>'
			.'<userdata name="phone">'.$rw->phone.'</userdata>'
			.'<userdata name="pan_no">'.$rw->pan_no.'</userdata>'
			.'<userdata name="country_id">'.$rw->country_id.'</userdata>'
			.'<userdata name="address">'.htmlspecialchars_decode($rw->address).'</userdata>'
			.'<userdata name="name">'.htmlspecialchars_decode($rw->name).'</userdata>';
			echo ' <cell>'.$rw->slno.'</cell>';
			echo ' <cell>'.$rw->name.'</cell>';
			echo ' <cell>'.$rw->gst_no.'</cell>';
			echo ' <cell>'.$rw->email.' - '.$rw->phone.'</cell>';
			echo ' <cell>'.$rw->status_val.'</cell>';           
            
            echo ' <cell style="display: flex;align-items: center;justify-content: space-between;"><![CDATA[';
            if ($is_edit == 1) {
            	echo '<div style="width:55px; display:flex; cursor:pointer;" onclick="preTally.AccountsTeam.editVendor('.$rw->id.');"><img src="images/icon/pencil.png" style="margin:2px 0;height:13px;"/> Edit</div>';
            }
            if ($rw->address != '') {
            	$viewdesc = '<div> Address: '.htmlspecialchars_decode($rw->address).'</div>';
            } else {
            	$viewdesc = '';
            }
            $viewdesc .= '<div> Type : '.$typeary[$rw->type].'</div>';
            $viewdesc .= '<div> Pan No : '.$rw->pan_no.'</div>';
            $viewdesc .= '<div> Country : '.htmlentities($rw->CN_Name).'</div>';
            if ($rw->updated_at != '') {
