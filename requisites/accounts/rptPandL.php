<?php
/**
* Profit and loss report with user filter (date, company, branch..)
...
$accObj->listRptPandL($inparams);
$result_data 		= $accObj->data_list;
$total_income 		= 0;
$total_expense 		= 0;

function xmlrowcreate($rw, $parent) {
	$listrow = '<row id="'.$rw['id'].'">';
	$listrow .= '<cell>'.($parent != '' ? htmlspecialchars_decode($parent).' -> ':' ').htmlspecialchars_decode($rw['title']).'</cell>';
	$listrow .= ' <cell>'.($rw['type'] == 1 ? number_format($rw['amount'],2,".",""):'0.00').'</cell>';
	$listrow .= ' <cell>'.($rw['type'] == 2 ? number_format($rw['amount'],2,".",""):'0.00').'</cell>';
	$listrow .= '</row>';
...
echo '<rows>';
	if (!empty($result_data)) {
		foreach($result_data as $rw) {
			echo xmlrowcreate($rw, '');
			$total_income 	+= ($rw['type'] == 1) ? $rw['amount']:0;
			$total_expense 	+= ($rw['type'] == 2) ? $rw['amount']:0;
			if (isset($rw['list'])) {
				foreach($rw['list'] as $rw1) {
					echo xmlrowcreate($rw1, trim($rw['title']));					
					$total_income 	+= ($rw1['type'] == 1) ? $rw1['amount']:0;
					$total_expense 	+= ($rw1['type'] == 2) ? $rw1['amount']:0;
				}
			}
		}
	} else {
