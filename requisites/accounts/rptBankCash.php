<?php
/**
 * Created BY Bilin @ 29-01-2026
 * Selected Bank based report or Company or branch wise cash transaction list 
 * 
*/
...
echo '<rows total_count="'.$total_records.'" pos="'.$inparams['start'].'">
    ...
    if (!empty($result_data)) {
		...
		foreach($result_data as $rw) {
			...
			echo ' <cell>'.htmlspecialchars_decode($rw->title).($rw->trackno != '' ? ' - Track: '.$rw->trackno : '').($rw->remarks != '' ? ' - '.$rw->remarks : '').'</cell>';
			echo ' <cell>'.date('M d, Y', strtotime($rw->date_entry)).'</cell>';
			echo ' <cell>'.number_format($rw->income,2,".","").'</cell>';
			echo ' <cell>'.number_format($rw->expense,2,".","").'</cell>';
			echo '</row>';
		}
	} else {
		echo '<row id="0"> <cell colspan="5"><![CDATA[<div style="font-size:16px;font-variant:small-caps;color:#0979B1;text-align:center !important;font-family: serif;padding-top: 10px;" >No records found.</div>]]></cell></row>';
	}
