...
$filter = ' TR.OF_Id = '.$preTally_user_ofid;
...
$BSRptObj->reportStatutoryData($filt,$filter,$REQUEST['f'],$REQUEST['t']);
$monthArray             = array("","January", "February", "March","April", "May", "June","July", "August", "September","October", "November", "December");
$Stat_Obj               = $BSRptObj->BranchArray;
...
echo("<?xml version='1.0' encoding='iso-8859-1'?>\n");
    ...
    echo '<head>
        ...
        <column width="120"  type="ro" align="left" ><![CDATA[<div><div id="branchStatItmF" style="width: 70%;float:left;" placeholder="Branch"></div><div style="float:left;margin-left:3px;margin-top:3px;"><img src="images/icon/sort-ascending-icon.png" title="Click here to Sort" colNum="1" class="bs_Sort_BIR"/></div></div>]]></column>';
    if($Stat_Obj && $BSRptObj->StatutoryReportArray) {
        ...
        foreach($Stat_Obj as $rw) {
            ...
            echo '<row id = "'.$rw->LC_Id.'">
                    ...
                    for($y = $toYear; $y >= $fromYear; $y--) {
                        ...
                        $arr_amts   = arrayFlatten($BSRptObj->StatMonthReportArray[$rw->LC_Id]);                        
                        $maxamt     = 0;
                        $maxamt     = max($arr_amts);
                        for($i = $monthLimit; $i >= $month; $i--) {
                            if($monthLimit == 0) 
                                $monthLimit = 12;
                            if($maxamt == $BSRptObj->StatMonthReportArray[$rw->LC_Id][$y][$i] && $maxamt > 0)
                                $bgcolor = 'style="background-color:#33cc33;"';
                            else
                                $bgcolor = ''; 
                            echo '<cell name="month'.$y.$i.'" title= " " '.$bgcolor.'>';
                            echo $amount = ($BSRptObj->StatMonthReportArray[$rw->LC_Id][$y][$monthLimit]) ? round($BSRptObj->StatMonthReportArray[$rw->LC_Id][$y][$monthLimit]) : '0';
                            echo '</cell>';
                            $monthLimit--;
                        }
                    }

                    echo '<cell name="BSAmount" title= " ">';
                    echo ($BSRptObj->StatutoryReportArray[$rw->LC_Id]->APS_StatutoryNAmt) ? round($BSRptObj->StatutoryReportArray[$rw->LC_Id]->APS_StatutoryNAmt) : '0';
                    echo '</cell>
            </row>';
            $j++;
        }
